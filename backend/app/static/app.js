...
async function runSimulation() {
    ...
    try {
        ...
        if (!res.ok) {
            ...
            throw new Error(`Simulation failed (HTTP ${res.status}): ${errText}`);
        }
        const data = await res.json();
...
function renderDecisionTrace(data) {
    ...
    if (eventIdSpan && data?.event?.event_id) {
    ...
    container.innerHTML = `
        ...
        <div class="bg-[#091224] border border-slate-800 rounded-lg p-3 text-xs space-y-1">
            ...
            <div class="pl-7 text-slate-300">
                Received error <code class="bg-slate-800 text-amber-300 px-1 py-0.5 rounded">${data.event?.error_code || 'N/A'}</code> for ${data.event?.customer?.name || 'Customer'} (Amount: <b>${formatINR(data.event?.amount)}</b>, Method: <b>${data.event?.payment_method}</b>, Retry Count: <b>${data.event?.retry_count || 0}</b>).
            </div>
        </div>
        ...
        <div class="bg-[#091224] border border-slate-800 rounded-lg p-3 text-xs space-y-1">
            <div class="flex items-center space-x-2 text-slate-400">
                ...
                <span class="ml-auto text-[10px] px-1.5 py-0.5 rounded bg-purple-500/10 text-purple-400 border border-purple-500/20 font-mono">Confidence: ${((diag.confidence_score || 0) * 100).toFixed(0)}%</span>
            </div>
            <div class="pl-7 text-slate-300 space-y-1">
                <div><b>Category:</b> <span class="text-purple-300">${diag.category || 'N/A'}</span> (${diag.is_transient ? 'Transient' : 'Permanent'})</div>
        ...
        <div class="bg-[#091224] border border-slate-800 rounded-lg p-3 text-xs space-y-1">
            ...
            <div class="pl-7 text-slate-300 space-y-1">
                <div><b>Action:</b> <span class="text-indigo-300 font-mono">${plan.action_type || 'N/A'}</span> on channel <b>${plan.channel || 'N/A'}</b> (Delay: ${plan.delay_minutes || 0}m)</div>
                <div><b>Unit Economics:</b> Est. Success: <b>${((plan.expected_success_probability || 0) * 100).toFixed(0)}%</b> | Cost: <b>${formatINR(plan.cost_of_intervention_inr)}</b></div>
                ${(plan.discount_percentage || 0) > 0 ? `<div class="text-amber-300 font-medium">Incentive: ${plan.discount_percentage}% bounded promo discount applied.</div>` : ''}
            </div>
        </div>
...
async function runBatchBenchmark() {
    ...
    try {
        ...
        if (!res.ok) {
            const errText = await res.text();
            throw new Error(`Benchmark failed (HTTP ${res.status}): ${errText}`);
        }
        ...
        if (tbody && summary.breakdown_by_category) {
            tbody.innerHTML = Object.entries(summary.breakdown_by_category).map(([cat, d]) => {
                const atRisk = Number(d?.at_risk_inr) || 0;
                const rec = Number(d?.recovered_inr) || 0;
                ...
                return `
                    <tr class="hover:bg-slate-800/40">
                        ...
                        <td class="p-2">${formatINR(atRisk)}</td>
                        <td class="p-2 text-emerald-400 font-semibold">${formatINR(rec)}</td>
