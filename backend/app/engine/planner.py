...
class InterventionPlanner:
    ...
    def create_plan(self, event: FailureEvent, diagnosis: DiagnosisResult) -> InterventionPlan:
        ...
        # Hard stop if retry count reached 3
        if event.retry_count >= 3:
            return InterventionPlan(
                ...
                explanation="Maximum 3 retry attempts exhausted. Stopping automated workflow to prevent customer fatigue.",
            )
        ...
        if diagnosis.category == FailureCategory.CHECKOUT_ABANDONMENT:
            # Grant bounded 5% discount only for High LTV or orders > ₹1500
            discount = 5.0 if (event.customer.segment == CustomerSegment.HIGH_LTV or event.amount >= 1500) else 0.0
            if discount > 0:
                msg = (
                    f"Hi {customer_name}! We noticed you left your items in the cart ({amount_fmt}). "
                    f"Complete your order in the next 2 hours and get an extra 5% instant discount: {{payment_link}}"
                )
        ...
        if diagnosis.category == FailureCategory.B2B_OVERDUE_INVOICE:
            if event.amount > 50000:
                msg = f"Accounts Payable Alert: Invoice #{event.order_id} of {amount_fmt} is overdue for {event.merchant_name}."
                return InterventionPlan(
                    plan_id=plan_id,
                    event_id=event.event_id,
                    ...
                    explanation="High-value B2B receivable (>₹50k) assigned to conversational AI Voice Dunning agent with promise-to-pay tracker.",
                )
            else:
                msg = (
